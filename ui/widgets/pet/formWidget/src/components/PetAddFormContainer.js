...
class PetAddFormContainer extends PureComponent {
  constructor(props) {
    super(props);
  ...
  async handleSubmit(pet) {
    ...
    if (authenticated) {
      try {
        const createdPet = await apiPetPost(serviceUrl, pet);
        onCreate(createdPet);
        this.setState({
    ...
    return (
      <>
        ...
        <AuthenticatedView keycloak={keycloak}>
          <PetForm onSubmit={this.handleSubmit} onCancelEditing={onCancelEditing} />
        </AuthenticatedView>
        <Notification
...
PetAddFormContainer.defaultProps = {
  onError: () => {},
  onCancelEditing: () => {},
  onCreate: () => {},
  serviceUrl: '',
...
export default withKeycloak(withTranslation()(PetAddFormContainer));
