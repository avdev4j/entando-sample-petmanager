...
const PetTable = ({ items, onSelect, classes, t, i18n, Actions }) => {
  const tableRows = items.map(item => (
    <TableRow hover className={classes.rowRoot} key={item.id} onClick={() => onSelect(item)}>
      ...
      <TableCell>
        <span>{new Date(item.birth).toLocaleString(i18n.language)}</span>
      </TableCell>
      {Actions && (
        <TableCell>
          <Actions item={item} />
  ...
  return items.length ? (
    ...
    <div className={classes.noItems}>{t('entities.pet.noItems')}</div>
  );
};

PetTable.propTypes = {
  items: PropTypes.arrayOf(petType).isRequired,
...
export default withStyles(styles)(withTranslation()(PetTable));
