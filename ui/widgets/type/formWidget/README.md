...
1. Go to widget source (<MICROSERVICE_FOLDER>/ui/widgets/type/formWidget/>):
    1. Run `npm i`
...
-   **type.form.createError** (fired when an error occurs during type creation)
-   **type.form.create** (fired when the type is successfully created)
-   **type.form.updateError** (fired when an error occurs during type update)
-   **type.form.update** (fired when the type is successfully updated)
