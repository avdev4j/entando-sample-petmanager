...
import { apiTypeGet, apiTypeDelete, apiTypePut } from 'api/types';
import Notification from 'components/common/Notification';
import TypeForm from 'components/TypeForm';

class TypeEditFormContainer extends PureComponent {
  constructor(props) {
    super(props);
  ...
  async handleSubmit(type) {
    ...
    if (authenticated) {
      try {
        const updatedType = await apiTypePut(serviceUrl, type);
  ...
  async handleDelete(type) {
    ...
    if (authenticated) {
      try {
        await apiTypeDelete(serviceUrl, type.id);
        onDelete(type);
        this.setState({
          type: null,
  ...
  render() {
    ...
    const { notificationMessage, notificationStatus, type } = this.state;

    let form;
    if (typeof type === 'undefined') {
      form = t('entities.type.notFound');
    } else if (type === null) {
      form = t('entities.type.deleted');
    } else {
      form = (
        <TypeForm
          type={type}
...
TypeEditFormContainer.propTypes = {
...
TypeEditFormContainer.defaultProps = {
  onCancelEditing: () => {},
  onDelete: () => {},
  onUpdate: () => {},
  onError: () => {},
...
export default withKeycloak(withTranslation()(TypeEditFormContainer));
