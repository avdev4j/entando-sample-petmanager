...
const TypeTable = ({ items, onSelect, classes, t, Actions }) => {
  ...
  return items.length ? (
    ...
    <div className={classes.noItems}>{t('entities.type.noItems')}</div>
  );
};

TypeTable.propTypes = {
  items: PropTypes.arrayOf(typeType).isRequired,
...
export default withStyles(styles)(withTranslation()(TypeTable));
